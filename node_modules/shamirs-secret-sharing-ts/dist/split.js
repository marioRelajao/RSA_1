...
// x = 0 ... n
// y = n ... 2n
const scratch = new Array(2 * constants_1.MAX_SHARES);
/**
...
function split(secret, opts) {
    ...
    if (!opts.shares || opts.shares < 0 || opts.shares > constants_1.MAX_SHARES) {
        throw new RangeError(`Shares must be 0 < shares <= ${constants_1.MAX_SHARES}.`);
    }
    ...
    if (!opts.random || 'function' !== typeof opts.random) {
        opts.random = random_1.random;
    }
    const hex = codec.hex(secret);
    const bin = codec.bin(hex, 16);
    // prepend 1 to get extra padding, we'll account for this later
    const parts = codec.split('1' + bin, constants_1.BIT_PADDING, 2);
    for (let i = 0; i < parts.length; ++i) {
        const p = points_1.points(parts[i], opts);
        for (let j = 0; j < opts.shares; ++j) {
            if (!scratch[j]) {
                scratch[j] = p[j].x.toString(16);
            }
            const z = p[j].y.toString(2);
            const y = scratch[j + constants_1.MAX_SHARES] || '';
            // y[j] = p[j][y] + y[j]
            scratch[j + constants_1.MAX_SHARES] = codec.pad(z) + y;
        }
    }
    for (let i = 0; i < opts.shares; ++i) {
        const x = scratch[i];
        const y = codec.hex(scratch[i + constants_1.MAX_SHARES], constants_1.BIN_ENCODING);
